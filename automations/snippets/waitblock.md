...
#### Simple Mode
For direct and uncomplicated delays, the Simple Mode lets you specify a fixed delay in days, hours, minutes, or seconds. This mode is ideal when you need a consistent pause in your flow. The requirement is to set at least one of these time units to a non-zero value.
![simple mode wait block](../images/wait-block-simple-mode.png)

#### Advanced Mode 
For scenarios where the delay needs to be more flexible, the Advanced Mode comes into play. This mode utilizes the [Expression Editor](/flow-editing/expressioneditor.html), which allows you to calculate the delay period dynamically. Whether through a variable, a property, or a custom expression, the resulting value determines the length of the pause, measured in seconds.
![advanced mode wait block](../images/wait-advanced-mode.png)
