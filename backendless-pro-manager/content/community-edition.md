# Backendless Pro Community Edition: Single-Node Installation Guide

The Backendless Pro Community Edition is meticulously engineered for deployment within a Kubernetes environment, focusing on a single-node installation strategy. This approach consolidates the entire Backendless ecosystem onto one node, optimizing for both efficiency and ease of access.
...
- **MySQL** (8+): Installed as `bl-mysql`.
- **Redis** (6+): Installed as `bl-redis`.
- **MongoDB** (5+): Installed as `bl-mongo`.
- **File System**: The necessary file system is mounted from the host machine into Kubernetes workloads, ensuring access to required files and directories.

## Key Features

Offering a suite of features designed to cater to the needs of both developers and businesses, the Backendless Pro Community Edition simplifies deployment and enhances productivity:

1. **Free Community Version**: Accessible at no cost, removing the barrier of licensing fees.
2. **All-in-One Node Deployment**: Simplifies setup and reduces operational complexities.
3. **Easy Management**: Allows for effortless configuration and monitoring through a centralized interface.
